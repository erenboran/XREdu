namespace XREdu.Unity.Demo.NewInputSystem.PlayerMovementWithInputAction.Character.Player.Movement
{
	using UnityEngine;
	...
	public class PlayerMovement : MonoBehaviour
	{
		...
		{
			get => this.runInputAction
					?? (this.runInputAction = this.inputActionAsset.FindActionMap("Player").FindAction("Run"));
			set => this.runInputAction = value;
		}

		protected InputAction MoveInputAction
		{
			get => this.walkInputAction
					?? (this.walkInputAction = this.inputActionAsset.FindActionMap("Player").FindAction("Walk"));
